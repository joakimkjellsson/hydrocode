...
!$OMP MASTER   
   ...
   IF ( (ihour == 0 .AND. iday == 1 .AND. imon == 1) .OR. ints == 1 ) THEN !Happy new year
      ...
      !! Define dimensions.
      !!
      WRITE ( ntimestring(1:10), '(i10)' ) ntime
...
#endif
#ifdef psirr      
      id_dims(1:6) = [id_mra, id_mra, id_lba, id_nst, id_nst, id_tim]
...
!$OMP DO REDUCTION(+:psrr,psrr2,volyr,volrr,psir,psir2,tsryz,tsryr,tsryr2,nyz,nyr,nyr2)
   DO jk=1,KM
