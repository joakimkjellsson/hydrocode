...
!$OMP END MASTER
   ...
   subroutine get_data_era5()
   !!---------------------------------------------------------------------------
   !!
   !! Read input data from ERA-5. 
   !! Written to read GRIB data on Mistral (DKRZ, Hamburg) 
   !! and convert to netCDF using CDO. 
   !!
   !! Popoulates arrays
   !! uflux, vflux, wflux, tem, sal, rho, geo, vol
   !! all of dimension IMT x JMT x KM
...
!$OMP MASTER
   ...
   !! File names
   !!
   !! u, v, t, z are stored in inDataDir/ml00_1H/<year>/E5ml00_1H_<year>-<mon>-<day>_<GRIBCODE>
   !! Convert with 
   !! cdo -P 8 -f nc -t ecmwf -remapbil,r360x180 -sp2gp,cubic -timselmean,6 
   !!
   !! q is stored same way, but is not spectral
   !! We do
   !! cdo -P 8 -f nc -t ecmwf -remapbil,r360x180 -setgridtype,regular -timselmean,6 
   !!
   !! Surface pressure is in inDataDir/sf00_1H/<year>/E5sf00_1H_<year>-<mon>_134
   !! We do
   !! cdo -P 8 -f nc -t ecmwf -remapbil,r360x180 -setgridtype,regular -timselmean,6 
   !!
   ...
   !! Find files
   dataprefix1 = '0000/E5ml00_1H_0000-00-00'
   dataprefix2 = '0000/E5sf00_1H_0000-00'
   
   ! Set year directory
   write (dataprefix1(1:4),   '(i4)') iyear
   write (dataprefix2(1:4),   '(i4)') iyear
   
   ! Set filenames
   write (dataprefix1(16:19), '(i4)') iyear
   write (dataprefix2(16:19), '(i4)') iyear
   write (dataprefix1(21:22), '(i2.2)') imon
   write (dataprefix2(21:22), '(i2.2)') imon
   write (dataprefix1(24:25), '(i2.2)') iday
   
   ! Set select command for CDO
   string = '-select,startdate=YYYY-MM-DDTHH:00:00,enddate=YYYY-MM-DDTHH:59:59'
   write (string(19:22), '(i4)') iyear
   write (string(24:25), '(i2.2)') imon
   write (string(27:28), '(i2.2)') iday
   write (string(30:31), '(i2.2)') ihour
   write (string(47:50), '(i4)') iyear
   write (string(52:53), '(i2.2)') imon
   write (string(55:56), '(i2.2)') iday
   write (string(58:59), '(i2.2)') ihour+5
   if (lverbose) then
       print*,' CDO select string ',TRIM(string)
   end if
   
   uFile = TRIM(inDataDir)//'/ml00_1H/'//TRIM(dataprefix1)//'_131'
   vFile = TRIM(inDataDir)//'/ml00_1H/'//TRIM(dataprefix1)//'_132'
   tFile = TRIM(inDataDir)//'/ml00_1H/'//TRIM(dataprefix1)//'_130'
   qFile = TRIM(inDataDir)//'/ml00_1H/'//TRIM(dataprefix1)//'_133'
   zFile = TRIM(inDataDir)//'/ml00_1H/'//TRIM(dataprefix1)//'_129'
   pFile = TRIM(inDataDir)//'/sf00_1H/'//TRIM(dataprefix2)//'_134'
   
   ! temporary file
   ! we will add _U, _V etc to this later
   !tmpFile = TRIM(tmpDataDir)//TRIM(dataprefix1(6:25))//TRIM(prefix)
   uFile2 = TRIM(tmpDataDir)//TRIM(dataprefix1(6:25))//'_'//string(30:31)//'_'//TRIM(prefix)//'_U'
   vFile2 = TRIM(tmpDataDir)//TRIM(dataprefix1(6:25))//'_'//string(30:31)//'_'//TRIM(prefix)//'_V'
   tFile2 = TRIM(tmpDataDir)//TRIM(dataprefix1(6:25))//'_'//string(30:31)//'_'//TRIM(prefix)//'_T'
   qFile2 = TRIM(tmpDataDir)//TRIM(dataprefix1(6:25))//'_'//string(30:31)//'_'//TRIM(prefix)//'_Q'
   zFile2 = TRIM(tmpDataDir)//TRIM(dataprefix1(6:25))//'_'//string(30:31)//'_'//TRIM(prefix)//'_Z'
   pFile2 = TRIM(tmpDataDir)//TRIM(dataprefix1(6:25))//'_'//string(30:31)//'_'//TRIM(prefix)//'_P'
   ...
   !!
   !! Use CDO to convert GRIB to netCDF (and regrid)
   !!
   cdo_command1 = 'cdo -O -f nc -t ecmwf -remapbil,era5_1deg_grid -sp2gp,cubic -timmean '//TRIM(string)
   cdo_command2 = 'cdo -O -f nc -t ecmwf -remapbil,era5_1deg_grid -setgridtype,regular -timmean '//TRIM(string)
   if (lverbose) then
       print*,' Executing for U '
       print*,TRIM(cdo_command1)//' '//TRIM(uFile)//' '//TRIM(uFile2)//'.nc > '//TRIM(uFile2)//'.log'
   end if

   ! Do not run these commands. Very slow
   ! Better to run cdo first to produce the netCDF files
   ! then run this code to read netCDF files
   ! See script 
   !call system(TRIM(cdo_command1)//' '//TRIM(uFile)//' '//TRIM(uFile2)//'.nc > '//TRIM(uFile2)//'.log')
   !call system(TRIM(cdo_command1)//' '//TRIM(vFile)//' '//TRIM(vFile2)//'.nc > '//TRIM(vFile2)//'.log')
   !call system(TRIM(cdo_command1)//' '//TRIM(tFile)//' '//TRIM(tFile2)//'.nc > '//TRIM(tFile2)//'.log')
   !call system(TRIM(cdo_command1)//' '//TRIM(zFile)//' '//TRIM(zFile2)//'.nc > '//TRIM(zFile2)//'.log')
   !call system(TRIM(cdo_command2)//' '//TRIM(qFile)//' '//TRIM(qFile2)//'.nc > '//TRIM(qFile2)//'.log')
   !call system(TRIM(cdo_command2)//' '//TRIM(pFile)//' '//TRIM(pFile2)//'.nc > '//TRIM(pFile2)//'.log')
   !call execute_command_line(TRIM(cdo_command1)//' '//TRIM(uFile)//' '//TRIM(uFile2)//'.nc > '//TRIM(uFile2)//'.log', wait=.true.)
   !call execute_command_line(TRIM(cdo_command1)//' '//TRIM(vFile)//' '//TRIM(vFile2)//'.nc > '//TRIM(vFile2)//'.log', wait=.true.)
   !call execute_command_line(TRIM(cdo_command1)//' '//TRIM(tFile)//' '//TRIM(tFile2)//'.nc > '//TRIM(tFile2)//'.log', wait=.true.)
   !call execute_command_line(TRIM(cdo_command1)//' '//TRIM(zFile)//' '//TRIM(zFile2)//'.nc > '//TRIM(zFile2)//'.log', wait=.true.)
   !call execute_command_line(TRIM(cdo_command2)//' '//TRIM(qFile)//' '//TRIM(qFile2)//'.nc > '//TRIM(qFile2)//'.log', wait=.true.)
   !call execute_command_line(TRIM(cdo_command2)//' '//TRIM(pFile)//' '//TRIM(pFile2)//'.nc > '//TRIM(pFile2)//'.log', wait=.true.)
   ...
   !!
   !! Get grid info from uFile
   !!   
   if (lverbose) print*,' Open ',TRIM(uFile2)//'.nc' 
   ierr = NF90_OPEN( TRIM(uFile2)//'.nc', NF90_SHARE, id_ncu)
   CALL err(ierr)
   ...
   CALL err(ierr)
   ierr = NF90_INQ_DIMID (id_ncu, 'lev', id_ml)  ! number of level midpoints (=KM)
   CALL err(ierr)
   ierr = NF90_INQ_DIMID (id_ncu, 'nhyi', id_hl) ! number of level interfaces (=KM+1)
   CALL err(ierr)
   ...
   IF (lfirst) THEN
      ! Read grid in the first time step
   ...
   ! Read a,b coeff from hybrid coord
   ...
   if (lverbose) print*,' Open file ',TRIM(vFile2)//'.nc'
   ierr = NF90_OPEN( TRIM(vFile2)//'.nc', NF90_SHARE, id_ncv)
   CALL err(ierr)
   if (lverbose) print*,' Open file ',TRIM(tFile2)//'.nc'
   ierr = NF90_OPEN( TRIM(tFile2)//'.nc', NF90_SHARE, id_nct)
   CALL err(ierr)
   if (lverbose) print*,' Open file ',TRIM(qFile2)//'.nc'
   ierr = NF90_OPEN( TRIM(qFile2)//'.nc', NF90_SHARE, id_ncq)
   CALL err(ierr)
   if (lverbose) print*,' Open file ',TRIM(zFile2)//'.nc'
   ierr = NF90_OPEN( TRIM(zFile2)//'.nc', NF90_SHARE, id_ncz)
   CALL err(ierr)
   if (lverbose) print*,' Open file ',TRIM(pFile2)//'.nc'
   ierr = NF90_OPEN( TRIM(pFile2)//'.nc', NF90_SHARE, id_ncp)
   CALL err(ierr)
   
   ! Variables
   ierr = NF90_INQ_VARID (id_ncu, 'u', id_u) 
   CALL err(ierr)
   ierr = NF90_INQ_VARID (id_ncv, 'v', id_v) 
   CALL err(ierr)
   ierr = NF90_INQ_VARID (id_nct, 't', id_t) 
   CALL err(ierr)
   ierr = NF90_INQ_VARID (id_ncq, 'q', id_q) 
   CALL err(ierr)
   ierr = NF90_INQ_VARID (id_ncz, 'z', id_z) 
   CALL err(ierr)
   ierr = NF90_INQ_VARID (id_ncp, 'SP', id_p) 
   CALL err(ierr)
   ...
   ! Read data
   ierr = NF90_GET_VAR (id_ncu, id_u, uxyz(:,0:jmt,:), start=[1,1,1,1], count=[IMT,JMT+1,KM,1]) !m/s
   CALL err(ierr)
   ierr = NF90_GET_VAR (id_ncv, id_v, vxyz(:,0:jmt,:), start=[1,1,1,1], count=[IMT,JMT+1,KM,1]) !m/s
   CALL err(ierr)
   ierr = NF90_GET_VAR (id_nct, id_t, txyz(:,0:jmt,:), start=[1,1,1,1], count=[IMT,JMT+1,KM,1]) !K
   CALL err(ierr)
   ierr = NF90_GET_VAR (id_ncq, id_q, qxyz(:,0:jmt,:), start=[1,1,1,1], count=[IMT,JMT+1,KM,1]) !kg/kg
   CALL err(ierr)
   ierr = NF90_GET_VAR (id_ncz, id_z, zxy(:,0:jmt), start=[1,1,1,1], count=[IMT,JMT+1,1,1]) !m2/s2
   CALL err(ierr)
   ierr = NF90_GET_VAR (id_ncp, id_p, pxy(:,0:jmt), start=[1,1,1,1], count=[IMT,JMT+1,1,1]) !Pa
   CALL err(ierr)
   
   if (lverbose) then
      !print*,'txyz(112,49,:)',txyz(112,49,:)
      !print*,'qxyz(112,49,:)',qxyz(112,49,:)
      !print*,'zxy(112,49)',zxy(112,49)
      !print*,'pxy(112,49)',pxy(112,49)
   ...
   !! Set top pressure explicitly
   !!
   rho_i(:,:,0) = 0.5 ! this can not be 0 since geopotential becomes infinite
   ...
   do jk=1,KM
      ...
      !! Mass fluxes
      !!
      call a2cgrid_u(uxyz(:,:,jk),uxyz_c(:,:,jk))
      uflux(:,:,jk) = (da + db * pu(:,:)) * uxyz_c(:,:,jk) * dy(:,:) / dg
       
      call a2cgrid_v(vxyz(:,:,jk),vxyz_c(:,:,jk))
      vflux(:,:,jk) = (da + db * pv(:,:)) * vxyz_c(:,:,jk) * dx(:,:) / dg
   ...
   do jk = 1, km
       do jj = 1, jmt
           do ji = 1, imt
                geo(ji,jj,jk) = 0.5 * (geo_i(ji,jj,jk-1) + geo_i(ji,jj,jk))
                rho(ji,jj,jk) = 0.5 * (rho_i(ji,jj,jk-1) + rho_i(ji,jj,jk))
           end do
       end do
   end do
   
   !if (lverbose) then
   !   print*,'tem(112,49,km)',tem(112,49,km),txyz(112,49,km)
   !   print*,'sal(112,49,km)',sal(112,49,km),qxyz(112,49,km)
   !   print*,'geo(112,49,km)',geo(112,49,km),zxy(112,49)
   !   print*,'rho(112,49,km)',rho(112,49,km),pxy(112,49)
   !end if
   !stop
   ...
   do jj = 1,JMT
       do ji = 1,IMT
           if (pt(ji,jj) < 50000) then 
               print*,'pt, ji, jj ',pt(ji,jj),ji,jj
               print*,'p',rho_i(ji,jj,:)
               stop
           end if
       end do
   ...
   deallocate ( txyz, qxyz, uxyz, vxyz, uxyz_c, vxyz_c, pxy, pt, pu, pv, zxy, aa, bb )
...
!$OMP END MASTER
   ...
   subroutine get_data_mri(run)
   !!---------------------------------------------------------------------------
   !!
   !! Subroutine to get data from MRI-GSMUV, atmospheric model of MRI-CGCM3.
   !! Data is stored every 6h with one file every month. 
   !!
   !!---------------------------------------------------------------------------
...
!$OMP MASTER
   ...
   if (lfirst .or. (iday == 1 .and. ihour == 0)) then
   ...
   if (istep == 1 .or. lfirst) then
      ...
      select case (trim(run))
         ...
         case ('HISTR')
            ...
            string='000000000000-000000001800.nc'
            ...
            tFile=trim(inDataDir)//trim(string)//'ta_'//trim(dataprefix)
            qFile=trim(inDataDir)//trim(string)//'hus_'//trim(dataprefix)
            zFile=trim(topoDir)//'orog_fx_MRI-CGCM3_historical_r0i0p0.nc'
            topoFile=TRIM(topoDir)//'areacella_fx_MRI-CGCM3_historical_r0i0p0.nc'
         ...
         case ('RCP85')
            ...
            tFile=trim(inDataDir)//trim(string)//'ta_'//trim(dataprefix)
            qFile=trim(inDataDir)//trim(string)//'hus_'//trim(dataprefix)
            zFile=trim(topoDir)//'orog_fx_MRI-CGCM3_rcp85_r0i0p0.nc'
            topoFile=TRIM(topoDir)//'areacella_fx_MRI-CGCM3_rcp85_r0i0p0.nc'
      ...
      if (tweak_zmean /= 1) then
         ...
         rstring = 'r000x000'
         write(rstring(2:4),'(i3.3)') imt
         write(rstring(6:8),'(i3.3)') jmt+1
